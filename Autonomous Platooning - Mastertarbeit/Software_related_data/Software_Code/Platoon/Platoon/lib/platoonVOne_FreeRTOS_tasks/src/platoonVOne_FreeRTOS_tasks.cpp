/**
 *
 * platoonVOne_FreeRTOS_tasks.h - Vehicle One freeRTOS tasks implementation
 * 
...
/* ****************** Global Variables ****************** */
static const char *TAG = "vOne_Task_handle";
...
void velocity_controller_task(void *arg)
{
	...
	uint32_t static cnt = 0;
		...
		// // Update cam msg data
		gVOneCamMsg.displacement = accumulated_displacement; // no need for "+=" since we do it on the "accumulated_displacement" var...
		gVOneCamMsg.velocity = d_values.velocity;
		gVOneCamMsg.acceleration = d_values.acceleration;
		gVOneCamMsg.target_velocity = r_v;
		gVOneCamMsg.target_distance = r_d;
		gVOneCamMsg.target_acceleration = r_a;
		xSemaphoreGive(binSemaphore);
		gVOneCamMsg.time_stamp_s = TIME_STAMP_SEC();
		gVOneCamMsg.time_stamp_ms = TIME_STAMP_MS();
		// // 3. Set new motor speed
		// // 3.1 when approaching curves slow down
		if (r_d < MINIMUM_SPACING_DIST)
		{
			new_velocity = gVOneCamMsg.velocity * (deceleration_ratio);
	...
	ESP_LOGD(TAG,"Displacement: %f",     gVOneCamMsg.displacement);
	ESP_LOGD(TAG,"Velocity: %f",         gVOneCamMsg.velocity);
	ESP_LOGD(TAG,"Acceleration:",        gVOneCamMsg.acceleration );
	ESP_LOGD(TAG,"Target_velocity:",     gVOneCamMsg.target_velocity);
	ESP_LOGD(TAG,"Target_distnace:",     gVOneCamMsg.target_distance);
	ESP_LOGD(TAG,"Target_acceleration:", gVOneCamMsg.target_acceleration);
	ESP_LOGD(TAG,"Time_stamp:",          gVOneCamMsg.time_stamp_s);
    ESP_LOGD(TAG,"Time_stamp:",          gVOneCamMsg.time_stamp_us);
#endif
	}
}
...
void data_logger_task(void *arg)
{
    ...
    while (1)
    {
		...
		case FREERTOS_TASK_NOTIFICATION_BIT:
			esp_now_send(broadcastAddress[LOGGER_INDEX], (uint8_t *)&gVOneCamMsg, sizeof(cam_message));
			break;
		case FREERTOS_RECEIVE_NOTIFICATION_BIT:
			esp_now_send(broadcastAddress[VEHICLE_TWO_MAC_INDEX], (uint8_t *)&gVOneCamMsg, sizeof(cam_message));
			break;
		default:break;
		}
